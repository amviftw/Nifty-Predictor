...
def render_sectoral_heatmap(snapshot: MarketSnapshot):
    ...
    for _, row in df.iterrows():
        ...
        chips.append(f'''<div style="
            ...
            padding:14px 16px;min-width:210px;flex:1 1 210px;max-width:280px;
        ">
            <div style="display:flex;justify-content:space-between;align-items:center;">
                <span style="font-weight:700;font-size:0.88rem;color:#e8ecf1;">{idx_name}</span>
                <span style="color:{chg_color};font-weight:600;font-size:0.88rem;">{change:+.2f}%</span>
            </div>
            <div style="font-size:1.0rem;font-weight:500;color:#c9cfd9;margin-top:4px;">
                {close:,.0f}</div>
            <div style="display:flex;gap:18px;margin-top:10px;font-size:0.72rem;">
                <div><span style="color:#7a8294;font-size:0.62rem;text-transform:uppercase;
                    letter-spacing:0.04em;">Week</span>
                    <br><span style="color:{wow_color};font-weight:500;">{wow:+.2f}%</span></div>
                <div><span style="color:#7a8294;font-size:0.62rem;text-transform:uppercase;
                    letter-spacing:0.04em;">Month</span>
                    <br><span style="color:{mom_color};font-weight:500;">{mom:+.2f}%</span></div>
            </div>
            {ad_html}
            {verdict_html}
        </div>''')
